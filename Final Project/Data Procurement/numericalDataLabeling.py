...
'''
This class reads in a cleaned dataset and labels it with technical patterns like chart patterns and candlestick patterns.
After implementing paper 2, this was given up on because paper 2 was time consuming and showed more promise in accomplishing
what was set out for this project.
'''
...
Using Object recoginition systems (pattern recognition) to find buy/sell signals in technical charts (Paper 3 was not implemented).
'''

def detect_double_top(df):
    '''
    This method detects double top patterns.
    '''
...
def detect_double_bottom(df):
    '''
    This method detects double bottom patterns.
    '''
...
def detect_head_and_shoulders(df):
    '''
    This method detects head and shoulders patterns. This does not really work currently.
    '''
...
def is_inverse_head_and_shoulders(df, index, lookback=30, tolerance=0.005):
    '''
    After implementing Paper 2,
    '''
    print("X")

def label_singular_trends(df):
    '''
    This method labels singular trends and is not really useful with what I am doing.
    '''
    trends = []
    current_trend = None
    trend_count = 1
...
def detect_ceiling_lines(df, threshold=0.001):
    '''
    This method detects ceilings in the stock.
    '''
    ...
    for i in range(1, len(df) - 1):
        if 'Peak' in df['Peak/Trough'].iloc[i] and (i + 7 < len(df)) and df['high'].iloc[i] >= df['high'].iloc[i + 1:i + 5].max():
            ...
            for j in range(i + 1, len(df)):
                ...
                is_peak = 'Peak' in df['Peak/Trough'].iloc[j]
...
def reverse_support_lines(df, threshold=0.001):
    '''
    Go backwards through the stock and label support lines.
    '''
...
def reverse_ceiling_lines(df, threshold=0.001):
    '''
    Go backwards through the stock and label ceilings.
    '''
    for i in range(len(df) - 2, 0, -1):
...
def detect_support_lines(df, threshold=0.001):
    '''
    Detect support lines found in the stock.
    '''
...
def label_peaks_troughs(df):
    '''
    Label peaks, troughs, and anomalies according to my desired specifications.
    '''
